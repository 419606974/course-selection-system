...
def permission_query():
	if 'role' in request.json:
		...
		if result:
			return jsonify({'success': True, 'result': [{'role':i['id'],'permission_ids':i['permission_ids']} for i in result]})
		else:
			return jsonify({'success': False, 'message': '权限查询失败'})
...
def permissions_update():
	...
	result=db.execute_sql('UPDATE role SET permission_ids = %s WHERE id = %s', (permissions_str, request.json["role"]))
	if result:
		if result:
