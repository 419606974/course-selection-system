...
def teacher_query():
    if request.args.get('query') is None and request.args.get('teacher_no') is None:
        teacher_infos = db.execute_sql('SELECT * from teacher ORDER BY id').fetchall()
...
def teacher_update():
    ...
    if not teacher_id:
        return jsonify({'success': False, 'message': '缺少教师ID参数！'})

    update_dict = {}

    if data.get("number") != '':
        update_dict.update({'number': data["number"]})
    if data.get("name") != '':
        update_dict.update({'name': data["name"]})
    if data.get("education") != '':
        update_dict.update({'education': data["education"]})
    ...
    if data.get("rank") != '':
        update_dict.update({'`rank`': data["rank"]})
    if data.get("sex") != '':
    ...
    cmd = f"UPDATE teacher SET {set_clause} WHERE id = %s"
    params = list(update_dict.values()) + [teacher_id]

    result = db.execute_sql(cmd, params)
    if result:
        insert_audit_log(request.remote_addr, current_user.id, model_name, f'更新教师信息: id={teacher_id}')
...
def delete_teacher():
    ...
    if not ids:
        return jsonify({'success': False, 'message': '未指定要删除的教师'})

    placeholders = ','.join(['%s'] * len(ids))
    sql = f"DELETE FROM teacher WHERE id IN ({placeholders})"
    result = db.execute_sql(sql, ids)

    if result:
        insert_audit_log(request.remote_addr, current_user.id, model_name, '删除教师信息: ' + str(ids))
